# MASE/COSIE code from github (plus getElbows helper function)
  ...
  if(par) {
    ...
    if(scaled.ASE) {
      latpos.list <- parLapply(cl = cl, 1:length(Adj_list), function(i)
        ase_JDA(Adj_list[[i]], d = d_vec[i], diag.augment = diag.augment, elbow = elbow_graph))
    }else{
      latpos.list <- parLapply(cl = cl, 1:length(Adj_list), function(i)
...
#' @author Jes\'us Arroyo <jesus.arroyo@jhu.edu>
#'
ase_JDA <- function(A, d = NA, d.max = sqrt(ncol(A)), diag.augment = TRUE, elbow = 1) {
  require(rARPACK)
...
#### getElbows ####
# function from Youngser Park's repo 'gmmase'
